...
import { format, isAfter, isToday, isTomorrow, startOfDay } from 'date-fns';
...
const CabBooking = ({ bookingTime }: CabBookingProps) => {
  ...
  const localSchema = Yup.object({
    ...
    bookingDate: bookingTime === 'later' ? Yup.date().required('Booking date is required') : Yup.date().nullable(),
  });
  ...
  const LocalForm = ({ type }: { type: 'local' | 'airport' | 'outstation' }) => {
    ...
    const formik = useFormik({
      initialValues: {
        ...
        luggage: 0,
        terminal: '',
        bookingDate: type === 'outstation' || bookingTime === 'later' ? new Date() : null,
      },
      ...
      onSubmit: async (values) => {
        ...
        try {
          const booking = await bookingAPI.createBooking({
            ...
            type: 'cab',
            category: type,
            date: values.bookingDate ? values.bookingDate.toISOString() : new Date().toISOString(),
          });
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !formik.values.bookingDate && 'text-muted-foreground',
                      formik.touched.bookingDate && formik.errors.bookingDate && 'border-destructive'
                    )}
                  >
                    <FaCalendarAlt className="mr-2" />
                    {formik.values.bookingDate ? format(formik.values.bookingDate, 'PPP') : 'Select date'}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={formik.values.bookingDate || undefined}
                    onSelect={(date) => formik.setFieldValue('bookingDate', date)}
  ...
  return (
    <div className="bg-card rounded-lg p-6 shadow-md">
      <h2 className="text-2xl font-semibold text-foreground mb-6">Book a Cab</h2>
